...
This document will build the modelpreviously fitted taking into account the measurement error observed with CTX-M at different plain counts and will also perform model diagnostics on the final model.
...
We will aim to get down to a q of 0.01, ie a minimum detectable prevalence of 1%.
...
# formula
form <- 'amp_c ~ co_amox+ s_housed_outdoor+f_pattern+  preweaned_heifer+
                 f_pneum_vacc+(1|farm)'
...
# create a function to produce auc
...
##crete dataframe of just the 1% prev results ie those which will be reported in the paper
...
```{r}
# specify models each with one variable missing

mv_1 <-glmer(amp_c ~ s_housed_outdoor+f_pattern+  preweaned_heifer+
                 f_pneum_vacc+(1|farm), data=df, family=binomial)

mv_2 <-glmer(amp_c ~ co_amox+ f_pattern+  preweaned_heifer+
                 f_pneum_vacc+(1|farm), data=df, family=binomial)

mv_3 <-glmer(amp_c ~ co_amox+ s_housed_outdoor+ preweaned_heifer+
                 f_pneum_vacc+(1|farm), data=df, family=binomial)

mv_4 <-glmer(amp_c ~ co_amox+ s_housed_outdoor+f_pattern+  
                 f_pneum_vacc+(1|farm), data=df, family=binomial)

mv_5 <-glmer(amp_c ~ co_amox+ s_housed_outdoor+f_pattern+  preweaned_heifer+
                 (1|farm), data=df, family=binomial)

# loop over each model to create a df of all model summarys
...
# plot coefficients for each model with error bars- if the error bars overlap for the coefficients on each model spec, we assume no collinearity 
