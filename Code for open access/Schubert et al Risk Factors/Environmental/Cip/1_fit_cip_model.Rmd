---
title: "fit_cip_measurement_error"
...
main_vars <- c('total_mg_pcu', 'ceph_3_4', 'fq', 'novobiocin', 's_temp')
re <- 'farm'
dv <- 'cipR'
...
form <- bf(cipR ~ (sens + spec - 1)/(1+exp(-eta)) + (1-spec),
          nl=TRUE)
form <- form + nlf(eta ~ main + reg + rand)
form <- form + lf(main ~ total_mg_pcu+ceph_3_4+fq+novobiocin+me(s_temp, 0.61), cmc=F)
