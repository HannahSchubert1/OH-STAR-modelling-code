...
This document will fit the Bayesian model.
...
main_vars <- c('total_mg_pcu', 'tet', 'fq', 'amox', 'cefalexin', 'strep', 's_temp')
re <- 'farm'
dv <- 'cephR'
...
form <- bf(cephR ~ (sens + spec - 1)/(1+exp(-eta)) + (1-spec),
          nl=TRUE)
form <- form + nlf(eta ~ main + reg + rand)
form <- form + lf(main ~ total_mg_pcu+ tet+ fq+ amox+ cefalexin+ strep+ me(s_temp, 0.61), cmc=F)
...
# define the priors for the two groups. 
