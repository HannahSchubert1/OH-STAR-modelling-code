...
The table below shows every variable with its OR, CI and adjusted p.

```{r include=FALSE}

vars <- names(df_with_na)

# loop over all variables

for(i in 7:length(vars))
{
# use trycatch so loop doesn't stop if there's an error
  tryTest = tryCatch(
  
# run the model
  {
    uni_model <- glmer(ctx_m ~ get(vars[i]) + (1 | farm), data=df_with_na, family=binomial)
    
    # create df of model output
...
summary <- as.data.frame(coef(summary(uni_model)))[-1, ]
 ...
 # if it's the first variable, create the df, otherwise join them together
if (i==7){
