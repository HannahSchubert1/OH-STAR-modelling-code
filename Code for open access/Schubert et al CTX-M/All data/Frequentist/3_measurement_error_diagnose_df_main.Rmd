...
The final model will use a q of 0.01 ie a minimum detectable prevalence of 1%.
...
# formula
form <- 'ctx_m ~ s_temp + s_housed_outdoor+f_foragetype_3 +  preweaned_heifer+(1|farm)'
...
# now loop through all the other models
...
##crete dataframe of just the 1% prev results
...
# run model each time with one variable missing
mv_1 <-glmer(ctx_m ~  s_housed_outdoor+f_foragetype_3 +  preweaned_heifer+(1|farm), data=df, family=binomial)

mv_2 <-glmer(ctx_m ~ s_temp + f_foragetype_3 +  preweaned_heifer+(1|farm), data=df, family=binomial)

mv_3 <-glmer(ctx_m ~ s_temp + s_housed_outdoor +  preweaned_heifer+(1|farm), data=df, family=binomial)

mv_4 <-glmer(ctx_m ~ s_temp + s_housed_outdoor+f_foragetype_3 +(1|farm), data=df, family=binomial)
...
for (i in 1:length(models)) {
    ...
    # create df with output from all above models
